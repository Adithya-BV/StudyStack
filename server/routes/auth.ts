...
export function isValidIITREmail(email: string): boolean {
  return /^[^\s@]+@([a-zA-Z0-9-]+\.)*iitr\.ac\.in$/.test(
  ...
  try {
    ...
    if (!isValidIITREmail(normalizedEmail)) {
      return res
        ...
        .json({
          error: "Only IIT Roorkee (*.iitr.ac.in) email addresses are allowed",
        })
    }
    ...
    res.json({
      ...
      message: "OTP sent to your IITR email",
...
authRouter.post("/verify-otp", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE users SET is_verified = 1 WHERE email = $1", [
      normalizedEmail,
    ])

    await pool.query("DELETE FROM otps WHERE email = $1 AND type = 'signup'", [
      normalizedEmail,
    ])
...
authRouter.post("/reset-password", async (req, res) => {
  try {
    ...
    await pool.query(
      "DELETE FROM otps WHERE email = $1 AND type = 'forgot_password'",
