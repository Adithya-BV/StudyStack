...
async function runTests() {
  ...
  console.log(`   ✅ Database seeded successfully with ${courses.length} courses.`);
  ...
  const record = db
    .prepare("SELECT * FROM otps WHERE email = ? AND otp_code = ? AND type = 'signup' AND expires_at > datetime('now')")
  ...
  db.prepare("DELETE FROM otps WHERE email = ? AND type = 'signup'").run(testEmail);
  ...
  const token = jwt.sign({ id: user.id, email: user.email, name: user.name }, "studystack_secret_jwt_key_2026_iitr", {
    expiresIn: "7d",
  });
  console.log(`   ✅ Password matched. JWT generated: ${token.substring(0, 20)}...`);
  ...
  // Pin it
  db.prepare("INSERT INTO pins (user_email, resource_id) VALUES (?, ?)").run(testEmail, resId);
  const pin = db.prepare("SELECT * FROM pins WHERE user_email = ? AND resource_id = ?").get(testEmail, resId);
  if (!pin) throw new Error("Pin failed");
  console.log(`   ✅ Created resource #${resId} and successfully pinned for ${testEmail}`);
